...
export async function analyzeBehavior(
	...
	const model = await settings.get<string>(SETTING_KEYS.AI_MODEL_BEHAVIOR_ANALYZER, "openai/gpt-4o")
...
export async function analyzeWebsite(
	...
	const model = await settings.get<string>(SETTING_KEYS.AI_MODEL_WEBSITE_ANALYZER, "openai/o3-mini")
	const maxRetries = await settings.get<number>(SETTING_KEYS.AI_RETRY_MAX_RETRIES, 3)
	const maxLength = await settings.get<number>(SETTING_KEYS.AI_LIMIT_WEBSITE_MARKDOWN_CHARS, 15000)
...
export async function buildCompanyProfile(
	...
	const model = await settings.get<string>(SETTING_KEYS.AI_MODEL_COMPANY_PROFILER, "openai/gpt-4.1-mini")
	const maxRetries = await settings.get<number>(SETTING_KEYS.AI_RETRY_MAX_RETRIES, 3)
	const temperature = await settings.get<number>(SETTING_KEYS.AI_TEMPERATURE_COMPANY_PROFILER, 0.5)
	const maxLength = await settings.get<number>(SETTING_KEYS.AI_LIMIT_COMPANY_CONTENT_CHARS, 10000)
